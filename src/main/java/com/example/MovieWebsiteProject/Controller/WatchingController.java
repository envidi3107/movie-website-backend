...
public class WatchingController {
    ...
    @GetMapping("/history")
    public ApiResponse<List<WatchingHistoryResponse>> getWatchingHistory(
                                                                         @RequestParam("type") String type) {
        List<WatchingHistoryResponse> history = watchingService.getWatchingHistoryByType(type);
        return ApiResponse.<List<WatchingHistoryResponse>>builder().code(SuccessCode.SUCCESS.getCode()).message(SuccessCode.SUCCESS.getMessage()).results(history).build();
    }
}
