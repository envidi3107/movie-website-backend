...
public class EpisodeCommentService {
    ...
    public CommentResponse saveComment(int episodeId, String parentCommentId, String content) {
        ...
        Episode ep = episodeRepository.findById(episodeId).orElseThrow(() -> new AppException(ErrorCode.FILM_NOT_FOUND));

        EpisodeComment parent = null;
        if (parentCommentId != null && !parentCommentId.isEmpty()) {
            parent = episodeCommentRepository.findById(parentCommentId).orElseThrow(() -> new AppException(ErrorCode.FAILED));
        }

        EpisodeComment comment = EpisodeComment.builder().user(user).episode(ep).content(content).commentTime(LocalDateTime.now()).parentComment(parent).build();
    ...
    public List<CommentResponse> getCommentsByEpisodeId(int episodeId) {
        List<EpisodeComment> comments = episodeCommentRepository.findByEpisode_IdAndParentCommentIsNullOrderByCommentTimeDesc(
                episodeId);
        return comments.stream().map(this::toDto).collect(Collectors.toList());
    }

    private CommentResponse toDto(EpisodeComment comment) {
        CommentResponse dto = CommentResponse.builder().commentId(comment.getCommentId()).userId(comment.getUser().getId()).username(comment.getUser().getUsername()).avatarPath(
                comment.getUser().getAvatarPath() != null ? baseUrl + comment.getUser().getAvatarPath() : null).content(comment.getContent()).commentTime(comment.getCommentTime()).build();
