...
public class CommentService {
    ...
    public CommentResponse saveComment(CommentRequest commentRequest) {
        ...
        if (count <= 2) {
            ...
            Comment comment = Comment.builder().user(user).film(film).content(commentRequest.getContent()).commentTime(LocalDateTime.now()).parentComment(parentComment).build();
            comment = commentRepository.save(comment);
            filmRepository.increaseComment(film.getFilmId());
            return CommentResponse.builder().commentId(comment.getCommentId()).userId(comment.getUser().getId()).username(comment.getUser().getUsername()).avatarPath(baseUrl + comment.getUser().getAvatarPath()).commentTime(comment.getCommentTime()).content(comment.getContent()).build();
        } else {
    ...
    private CommentResponse convertToDTO(Comment comment) {
        return CommentResponse.builder().commentId(comment.getCommentId()).userId(comment.getUser().getId()).username(comment.getUser().getUsername()).avatarPath(baseUrl + comment.getUser().getAvatarPath()).content(comment.getContent()).commentTime(comment.getCommentTime()).childComments(
                comment.getChildComments() == null ? List.of() : comment.getChildComments().stream().map(this::convertToDTO).collect(Collectors.toList())).build();
    }
}
