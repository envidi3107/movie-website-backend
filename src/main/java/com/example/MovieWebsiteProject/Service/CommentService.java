...
public class CommentService {
  ...
  public CommentResponse saveComment(CommentRequest commentRequest) {
    ...
    if (count <= 2) {
      Film film =
          filmRepository
              .findById(commentRequest.getFilmId())
              .orElseThrow(() -> new RuntimeException("Film not found"));

      Comment parentComment = null;

      if (commentRequest.getParentCommentId() != null
          && !commentRequest.getParentCommentId().isEmpty()) {
        parentComment =
            commentRepository
                .findById(commentRequest.getParentCommentId())
  ...
  public List<CommentResponse> getCommentsByFilmId(String filmId) {
    ...
    List<Comment> parentComments =
        commentRepository.findByFilm_FilmIdAndParentCommentIsNullOrderByCommentTimeDesc(filmId);
