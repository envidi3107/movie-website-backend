...
public class EpisodeReactionService {
    ...
    public void saveEpisodeReaction(String userId, int episodeId, String reactionType) {
        ...
        episodeReactionRepository.getByUserAndEpisode(userId, episodeId).ifPresentOrElse(
                record -> {
                    if (record.getReactionType().equalsIgnoreCase(type)) {
                        episodeReactionRepository.delete(record);
                        // decrement counts
                        if (type.equals(ReactionType.LIKE.name()) && ep.getLikeCount() > 0)
                            ep.setLikeCount(ep.getLikeCount() - 1);
                        if (type.equals(ReactionType.DISLIKE.name()) && ep.getDislikeCount() > 0)
