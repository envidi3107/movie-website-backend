...
public class WatchingHistoryResponse {
    ...
    public WatchingHistoryResponse(Watching watching) {
        ...
        this.film = FilmSummaryResponse.builder().filmId(filmEntity.getFilmId()).title(filmEntity.getTitle()).adult(filmEntity.isAdult()).type(filmEntity.getType()).releaseDate(filmEntity.getReleaseDate()).rating(filmEntity.getRating()).numberOfViews(filmEntity.getNumberOfViews()).backdropPath(filmEntity.getBackdropPath()).posterPath(filmEntity.getPosterPath()).genres(
                filmEntity.getGenres().stream().map(Genre::getGenreName).collect(Collectors.toSet())).build();
